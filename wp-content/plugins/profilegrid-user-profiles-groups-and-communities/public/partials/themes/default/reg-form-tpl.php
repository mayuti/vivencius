...
<?php 
if ( is_user_logged_in() && $group_member && !has_action('profilegrid_group_register_onlogin')) : ?>
	<?php
    ...
    <div class="pm-login-box pm-dbfl pm-radius5 pm-border"> 
	  <div class="pm-login-header pm-dbfl pm-bg pm-border-bt">
	   ...
	   <div class="pm-login-header-buttons pm-dbfl pm-pad10">
		   <div class="pm-center-button pm-difl pm-pad10"><a href="<?php echo esc_attr($redirect_url);?>" class="pm_button"><?php esc_html_e('My Profile','profilegrid-user-profiles-groups-and-communities');?></a></div>
		   <div class="pm-center-button pm-difl pm-pad10"><a href="<?php echo esc_attr(wp_logout_url( $pmrequests->profile_magic_get_frontend_url('pm_user_login_page',''))); ?>" class="pm_button"><?php esc_html_e('Logout','profilegrid-user-profiles-groups-and-communities');?></a></div>
	   </div>
       </div>
       </div>
	 <?php
...
<form class="pmagic-form pm-dbfl" method="post" action="" id="pm_regform_<?php echo esc_attr($gid); ?>" name="pm_regform_<?php echo esc_attr($gid); ?>" onsubmit="return profile_magic_frontend_validation(this)" enctype="multipart/form-data">
  ...
  ?>
    ...
    */
    ...
    <?php endif;  } endif; ?>
        <input type="submit" value="<?php esc_attr_e('Submit','profilegrid-user-profiles-groups-and-communities');?>" name="reg_form_submit">
    </div>
  </form>
  </div>
</div>
